...
from .version import read_dcode_version_from
from .version import read_workspace_version_from
from .version import update_dcode_snapshot_versions
from .version import update_dcode_version
from .version import update_workspace_version


class VersionTest(unittest.TestCase):
    def test_updates_only_dcode_product_version(self) -> None:
        with TemporaryDirectory() as directory:
            product_source = Path(directory) / "lib.rs"
            product_source.write_text(
                'pub const DCODE_VERSION: &str = "1.2.3";\n'
                'pub const UNRELATED: &str = "9.9.9";\n',
                encoding="utf-8",
            )

            version = update_dcode_version(product_source, "patch")

            self.assertEqual(version, "1.2.4")
            self.assertEqual(read_dcode_version_from(product_source), "1.2.4")
            self.assertIn(
                'pub const UNRELATED: &str = "9.9.9";',
