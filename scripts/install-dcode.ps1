...
function Assert-ReleasePackage([string]$PackageDir, [string]$ExpectedVersion) {
    ...
    $runtimeBinary = Join-Path $PackageDir "bin\codex.exe"
    if (-not (Test-Path -LiteralPath $runtimeBinary -PathType Leaf)) { throw "Package does not contain bin\codex.exe runtime" }
