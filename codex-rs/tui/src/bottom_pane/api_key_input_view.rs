...
type ApiKeySubmitted = Box<dyn Fn(String) + Send + Sync>;

/// Single-line secret input shared by provider-specific login flows.
...
impl ApiKeyInputView {
    pub(crate) fn new(provider_name: String, on_submit: ApiKeySubmitted) -> Self {
        Self {
            provider_name,
            value: String::new(),
            error: None,
    ...
    fn masked_value(&self, width: u16) -> String {
        let max_symbols = usize::from(width.saturating_sub(4));
...
impl Renderable for ApiKeyInputView {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        let lines = vec![
            Line::from(vec![
                "▌ ".cyan(),
                format!("{} login", self.provider_name).bold(),
            ]),
