...
pub struct Cli {
    ...
    // Internal controls set by the top-level `dcode resume` subcommand.
    ...
    /// top-level `dcode resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `dcode fork` subcommand.
    // These are not exposed as user flags on the base `codex` command.
    #[clap(skip)]
    ...
    /// top-level `dcode fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    /// Disable full-screen mode and render inline instead
    ///
