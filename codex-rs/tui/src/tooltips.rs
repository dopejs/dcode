...
const APP_TOOLTIP: &str = "Try the **Desktop app**. Run 'dcode app' or visit https://chatgpt.com/codex?app-landing-page=true";
const FAST_TOOLTIP: &str =
    "*New* Use **/fast** to enable our fastest inference with increased plan usage.";
const OTHER_TOOLTIP: &str = "*New* Build faster with the **Desktop app**. Run 'dcode app' or visit https://chatgpt.com/codex?app-landing-page=true";
const OTHER_TOOLTIP_NON_MAC: &str = "*New* Build faster with DCode.";
const FREE_GO_TOOLTIP: &str =
    "*New* For a limited time, DCode is included in your plan for free – let’s build together.";
...
lazy_static! {
    static ref TOOLTIPS: Vec<&'static str> = RAW_TOOLTIPS
        ...
        .filter(|line| {
            ...
            if !IS_MACOS && !IS_WINDOWS && line.contains("dcode app") {
                return false;
            }
            true
        })
...
/// Pick a random tooltip to show to the user when starting DCode.
    ...
    impl TargetOs {
        const fn current() -> Self {
            if cfg!(target_os = "macos") {
                ...
                // DCode currently publishes CLI builds for macOS, Windows, and Linux.
                Self::Linux
            }
        }
    }
...
from_date = "2000-01-01"
    ...
    fn announcement_tip_toml_parse_comments() {
        let toml = r#"
# Example announcement tips for DCode TUI.
...
[[announcements]]
content = "Welcome to DCode! Check out the new onboarding flow."
