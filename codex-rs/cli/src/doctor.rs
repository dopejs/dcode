//! Implements the `dcode doctor` diagnostic report.
//!
...
/// Options for building a local DCode diagnostic report.
///
...
///
/// This is the CLI entry point for dcode doctor. It does not repair issues;
            ...
            let (
                ...
                async {
                    run_sync_check("config", progress.clone(), || {
                        ...
                        .remediation("Fix the reported config error, then rerun dcode doctor.")
                    })
                },
                async { run_sync_check("network", progress.clone(), network_check) },
                async {
...
async fn load_config(
    ...
    ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load DCode config")
}
...
/// JSON support report emitted by `dcode doctor --json`.
///
...
fn installation_check(show_details: bool) -> DoctorCheck {
    ...
    if path_entries.len() > 1 {
        details.push(format!("PATH dcode entries: {}", path_entries.len()));
    }
    if show_details || path_entries.len() > 1 {
        details.extend(
            path_entries
                .iter()
                .enumerate()
                .map(|(index, path)| format!("PATH dcode #{}: {path}", index + 1)),
        );
    }
...
fn codex_path_entries() -> Vec<String> {
    #[cfg(windows)]
    let result = run_command("where", ["dcode"]);
    #[cfg(not(windows))]
    let result = run_command("which", ["-a", "dcode"]);
    ...
    match load_auth_dot_json(
        ...
        Ok(Some(auth)) => {
            ...
            if status == CheckStatus::Fail {
                check =
                    check.remediation("Run dcode login again or provide a supported auth env var.");
            }
            check
        }
        ...
        Ok(None) => DoctorCheck::new(
            ...
            "auth",
            CheckStatus::Fail,
            "no DCode credentials were found",
        )
        .details(details)
        .remediation("Run dcode login or provide an API key through a supported auth env var."),
        ...
        .detail(err.to_string())
        .remediation("Fix auth storage access or run dcode login again."),
    }
}
...
mod tests {
    ...
    fn provider_specific_auth_fails_when_provider_env_key_is_missing() {
        let check = provider_specific_auth_check(
            ...
            Some("Set PROVIDER_API_KEY before running DCode."),
            Vec::new(),
            |_| false,
        )
        .expect("non-OpenAI provider should produce a provider-specific check");
        ...
        assert_eq!(
            check.remediation,
            Some("Set PROVIDER_API_KEY before running DCode.".to_string())
        );
    }
