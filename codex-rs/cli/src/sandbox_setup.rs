...
pub(crate) struct SandboxSetupCommand {
    ...
    /// Windows user that will run DCode after managed deployment.
    #[arg(
        long = "user",
        value_name = "USER",
    ...
    /// Use the current Windows user as the DCode user.
    #[arg(
        long = "current-user",
    ...
    /// CODEX_HOME for the DCode user. Required with --user.
...
impl SandboxSetupCommand {
    fn setup_level(&self) -> anyhow::Result<SandboxSetupLevel> {
        if self.elevated_sandbox_level {
            ...
            anyhow::bail!("`dcode sandbox setup` currently requires --elevated");
        }
    }
}
...
async fn run_elevated(
    ...
    let config = ConfigBuilder::default()
        ...
        .build()
        .await
        .context("failed to load target user's DCode config for sandbox provisioning")?;
