...
"#
    ...
    let mut command = tokio::process::Command::new(codex_utils_cargo_bin::cargo_bin("dcode")?);
    command
...
"#
    ...
    let codex_bin = codex_utils_cargo_bin::cargo_bin("dcode")?;
...
fn local_exec_server_exits_successfully_on_sigterm() -> Result<()> {
    let codex_home = TempDir::new()?;
    let mut child = std::process::Command::new(codex_utils_cargo_bin::cargo_bin("dcode")?)
        .env("CODEX_HOME", codex_home.path())
