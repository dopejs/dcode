...
const DEEPSEEK_REQUEST_TIMEOUT: Duration = Duration::from_secs(5);

/// Credential storage settings used for DeepSeek account requests outside an active model session.
#[derive(Clone, Debug)]
pub struct DeepSeekAuthStorageConfig {
...
struct DeepSeekModelsManager {
    endpoint: DeepSeekModelsEndpoint,
...
impl DeepSeekModelsManager {
    fn new(
        ...
        Self {
            endpoint: DeepSeekModelsEndpoint::new(provider_info, auth_manager.clone()),
    ...
    async fn refresh(&self, http_client_factory: HttpClientFactory) {
        match self.endpoint.list_models_inner(http_client_factory).await {
            Ok((models, _etag)) if !models.is_empty() => *self.models.write().await = models,
            Ok(_) => tracing::warn!(
                model = DEEPSEEK_DEFAULT_MODEL,
                "DeepSeek /models did not return the supported model; using bundled metadata"
            ),
...
impl DeepSeekModelsEndpoint {
    ...
    async fn list_models_inner(
        ...
        let response =
            get_deepseek_json(&self.provider_info, api_auth, "models", http_client_factory).await?;
...
impl ModelsEndpointClient for DeepSeekModelsEndpoint {
    ...
    fn list_models<'a>(
        ...
        http_client_factory: HttpClientFactory,
    ) -> ModelsEndpointFuture<'a, CodexResult<(Vec<ModelInfo>, Option<String>)>> {
        Box::pin(DeepSeekModelsEndpoint::list_models_inner(
            self,
            http_client_factory,
        ))
    }
...
/// Validates a DeepSeek API key without persisting it.
///
/// A successful balance response proves that the key is accepted by the configured
/// DeepSeek endpoint. Callers should persist the key only after this returns `Ok`.
...
/// Fetches a DeepSeek balance using an explicit API key.
pub async fn get_deepseek_balance_with_api_key(
    provider_info: &ModelProviderInfo,
    api_key: &str,
    http_client_factory: HttpClientFactory,
) -> CodexResult<ProviderBalance> {
    let auth = crate::auth::auth_provider_from_auth(&CodexAuth::from_api_key(api_key));
    get_balance(provider_info, auth, http_client_factory).await
}

/// Fetches a DeepSeek balance using the environment key when present, otherwise stored auth.
pub async fn get_deepseek_balance_with_stored_auth(
    provider_info: &ModelProviderInfo,
    auth_storage: &DeepSeekAuthStorageConfig,
    http_client_factory: HttpClientFactory,
) -> CodexResult<ProviderBalance> {
    match resolve_provider_auth(/*auth*/ None, provider_info) {
        Ok(auth) => return get_balance(provider_info, auth, http_client_factory).await,
        Err(err) if matches!(err.details(), CodexErrorDetails::EnvVar(_)) => {}
        Err(err) => return Err(err),
    }

    let auth = CodexAuth::from_auth_storage(
        &auth_storage.codex_home,
        auth_storage.auth_credentials_store_mode,
        auth_storage.chatgpt_base_url.as_deref(),
        auth_storage.keyring_backend_kind,
        &auth_storage.auth_route_config,
    )
    .await?;
    let auth = resolve_provider_auth(auth.as_ref(), provider_info)?;
    get_balance(provider_info, auth, http_client_factory).await
}
...
fn deepseek_flash_model() -> ModelInfo {
    ...
    model.description = Some("Latest frontier agentic coding model.".to_string());
    ...
    model.comp_hash = Some("3000".to_string());
